...
import { CheckCircle2, ScanLine, AlertCircle, Edit, Trash2, Eye, MessageSquare } from 'lucide-react';
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5 text-sm">
                {clients.length > 0 ? clients.map((client) => (
                    <tr
                      ...
                      className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors cursor-pointer select-none"
                      title="Double-cliquer pour voir le profil complet"
                    >
                        <td className="p-4">
                        ...
                        <td className="p-4 text-right flex items-center justify-end gap-2">
                            <button onClick={e => { e.stopPropagation(); onView(client); }} className="p-2 text-slate-400 hover:text-brand-teal hover:bg-brand-teal/10 rounded-lg transition-colors" title="Voir le profil complet">
                                <Eye className="w-4 h-4" />
                            </button>
                            {onContact && (
                              <button onClick={e => { e.stopPropagation(); onContact(client); }} className="p-2 text-slate-400 hover:text-brand-blue hover:bg-brand-blue/10 rounded-lg transition-colors" title="Voir la messagerie de ce client">
                                <MessageSquare className="w-4 h-4" />
                              </button>
                            )}
                            <button onClick={e => { e.stopPropagation(); onEdit(client); }} className="p-2 text-slate-400 hover:text-brand-blue hover:bg-brand-blue/10 rounded-lg transition-colors" title="Modifier Profil">
                                <Edit className="w-4 h-4" />
                            </button>
                            <button onClick={e => { e.stopPropagation(); onDelete(client.id); }} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-100 rounded-lg transition-colors" title="Supprimer">
