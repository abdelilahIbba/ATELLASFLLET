          ...
          {roles.map((role) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-1.5 min-w-0">
                <h4 className={`font-bold leading-snug truncate flex-1 min-w-0 ${selectedRole.id === role.id ? 'text-white' : 'text-brand-navy dark:text-white'}`}>
                  {role.name}
                </h4>
                {role.isSystem && (
                  <Lock className={`w-3 h-3 flex-shrink-0 ${selectedRole.id === role.id ? 'text-white/60' : 'text-slate-400'}`} />
                )}
              </div>
              {/* Row 2: user count badge */}
              <div className="mb-2">
