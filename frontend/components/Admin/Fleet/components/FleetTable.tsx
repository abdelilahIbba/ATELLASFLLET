...
import { Fuel, Gauge, Edit, Trash2, AlertCircle, AlertTriangle, CheckCircle2 } from 'lucide-react';
    ...
    <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5 text-sm">
                    ...
                    <tr key={vehicle.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                        ...
                        <td className="p-4">
                            <div className="flex items-center gap-4 text-xs text-slate-500">
                                ...
                                <div className="flex items-center gap-1" title="Quantité">
                                    <span className="inline-flex items-center gap-1 bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 font-bold px-2 py-0.5 rounded text-[10px]">
                                        ×{vehicle.quantity ?? 1}
                                    </span>
                                </div>
                            </div>
                        ...
                        <td className="p-4">
                          <div className="flex flex-col gap-1">
                            {([
                              { key: 'insurance',       label: 'Ass.' },
                              { key: 'visiteTechnique', label: 'VT'  },
                              { key: 'vignette',        label: 'Vig.' },
                              { key: 'carteGrise',      label: 'CG'  },
                            ] as const).map(({ key, label }) => {
                              const d = vehicle.documents?.[key] || '';
                              let s: ReturnType<typeof getExpiryStatus> | null = null;
                              try { s = d ? getExpiryStatus(d) : null; } catch {}
                              if (!s) return (
                                <span key={key} className="flex items-center gap-1 text-[10px] text-slate-300 dark:text-slate-600">
                                  <span className="w-1.5 h-1.5 rounded-full bg-slate-200 dark:bg-slate-600 inline-block" />{label}
                                </span>
                              );
                              const Icon = s.status === 'Valid' ? CheckCircle2 : s.status === 'Warning' || s.status === 'Notice' ? AlertTriangle : AlertCircle;
                              return (
                                <span key={key} title={`${label}: ${d} (${s.label})`}
                                  className={`flex items-center gap-1 text-[10px] font-bold ${s.color.split(' ')[0]}`}>
                ...
                <tr>
                    <td colSpan={7} className="p-8 text-center text-slate-500 italic">Aucun véhicule trouvé correspondant aux critères.</td>
                </tr>
                )}
            </tbody>
        </table>
