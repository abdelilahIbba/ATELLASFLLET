...
interface AuthModalProps {
  isOpen: boolean;
  onClose: () => void;
  /** Returns true on success, false on failure (wrong creds or wrong role tab) */
  onLogin?: (email: string, password: string, role: 'client' | 'admin') => Promise<boolean>;
...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onLogin, onRegister }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      if (!onLogin) { setIsLoading(false); return; }
      const success = await onLogin(formData.email, formData.password, role);
      setIsLoading(false);
      if (success) {
        onClose();
        ...
        setError(
          role === 'admin'
            ? "Accès refusé. Identifiants invalides ou rôle non autorisé."
      ...
      if (!onRegister) { setIsLoading(false); onClose(); return; }
      const errorMsg = await onRegister(formData.name, formData.email, formData.password);
      setIsLoading(false);
      if (errorMsg === null) {
        onClose();
        setFormData({ name: '', email: '', password: '', confirmPassword: '' });
           ...
           <div className="max-w-md mx-auto w-full">
             ...
             <AnimatePresence mode="wait">
               ...
               >
                  ...
                  {error && (
                    <div className="text-center text-[14px] text-red-500 px-3 py-2 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                      {error}
                    </div>
                  )}
                  {!isLogin && error && null /* handled above */}
