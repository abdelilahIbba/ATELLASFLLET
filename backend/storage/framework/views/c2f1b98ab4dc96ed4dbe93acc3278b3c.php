<nav x-data="{ isOpen: false }" class="sticky top-0 z-30 py-4 px-4 lg:px-10">
    <div class="max-w-6xl mx-auto">
        <div class="flex items-center justify-between rounded-2xl border border-white/10 bg-slate-900/70 px-4 lg:px-8 py-3 text-slate-100 shadow-2xl shadow-slate-900/40 backdrop-blur-2xl">
            <a href="<?php echo e(route('home')); ?>" class="text-2xl font-black tracking-widest">
                STE BEL <span class="text-cyan-300">ESPACE</span> CAR
            </a>

            <div class="hidden lg:flex items-center gap-8 text-sm font-semibold">
                <a href="<?php echo e(route('home')); ?>"
                    class="transition hover:text-cyan-300 <?php echo e(request()->routeIs('home') ? 'text-cyan-300' : 'text-gray-200'); ?>">Home</a>
                <a href="<?php echo e(route('cars.showAll')); ?>"
                    class="transition hover:text-cyan-300 <?php echo e(request()->routeIs('cars.showAll') ? 'text-cyan-300' : 'text-gray-200'); ?>">Cars</a>
                <a href="<?php echo e(route('blogs.index')); ?>"
                    class="transition hover:text-cyan-300 <?php echo e(request()->routeIs('blogs.*') ? 'text-cyan-300' : 'text-gray-200'); ?>">Blog</a>
            ...
            <div class="hidden lg:flex items-center gap-3">
                <?php if(auth()->guard()->check()): ?>
                    <div class="text-sm text-gray-200">Hello, <?php echo e(Auth::user()->name); ?></div>
        ...
        <div x-show="isOpen" x-transition class="lg:hidden mt-3 rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-6 text-sm text-gray-100 shadow-xl shadow-slate-900/40 backdrop-blur-2xl">
            <div class="grid gap-4">
                <a href="<?php echo e(route('home')); ?>" class="hover:text-cyan-300">Home</a>
                <a href="<?php echo e(route('cars.showAll')); ?>" class="hover:text-cyan-300">Cars</a>
                <a href="<?php echo e(route('blogs.index')); ?>" class="hover:text-cyan-300">Blog</a>
                <a href="#about" class="hover:text-cyan-300">About</a>
                <a href="#contact" class="hover:text-cyan-300">Contact</a>
            </div>
            <div class="mt-6 grid gap-3">
                <?php if(auth()->guard()->check()): ?>
                    <span class="text-sm text-gray-200">Hello, <?php echo e(Auth::user()->name); ?></span>
                    <form method="POST" action="<?php echo e(route('logout')); ?>">
                        <?php echo csrf_field(); ?>
                        <button type="submit" class="w-full py-2 rounded-full bg-white/10 text-sm">Logout</button>
                    </form>
                <?php else: ?>
                    <a href="<?php echo e(route('login')); ?>" class="w-full text-center py-2 rounded-full bg-white/10">Login</a>
                    <a href="<?php echo e(route('register')); ?>"
...
<?php /**PATH /var/www/html/resources/views/cars/partials/nav.blade.php ENDPATH**/ ?>